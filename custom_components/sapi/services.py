...
class Services:
    ...
    def __init__(
        self, coordinator: SAPIDataUpdateCoordinator, hass: HomeAssistant
    ) -> None:
        self.hass = hass
    ...
    async def get_date_today(self, _: ServiceCall) -> None:
        ...
        try:
            ...
            if data:
                self.hass.states.async_set(f"sensor.{SERVICE_DATE_TODAY}", data)
    ...
    async def generate_password(self, call: ServiceCall) -> JsonObjectType:
        ...
        try:
            ...
            _LOGGER.debug(
                "Generated password with length %s with special %s",
                length,
                include_special,
            )
